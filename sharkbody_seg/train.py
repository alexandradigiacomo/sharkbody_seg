...
if __name__ == '__main__':
    ...
    for epoch in range(1, cfg['epochs'] + 1):
        ...
        with tqdm(total=len(train_set),
            ...
            for i, batch in enumerate(train_loader):
                ...
                iou_score = iou[0].item() # iou from tensor to scalar
        ...
        if val_iou_score > best_val_iou_score:
            best_val_iou_score = val_iou_score # update val iou
            best_epoch = epoch
            best_checkpoint = model.state_dict()
            logging.info(f"New best IoU: {best_val_iou_score}")
    ...
    print(f'best checkpoint {best_checkpoint} saved! Good job DiG! Lets have a Kombucha!')
    if best_checkpoint is not None:
        checkpoint_filename = f'bestcheckpoint_epoch{best_epoch}.pth'
        torch.save({
            'state_dict': best_checkpoint,
            'best_iou_score': best_val_iou_score,
