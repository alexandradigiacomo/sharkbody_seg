...
from sharkbody_seg.dataset import SharkBody, UnlabeledSharkBody
...
def get_args():
    ...
    parser.add_argument('--split', type=str, default=None, # data split for model train/val/test
                        help='Define data split')
    parser.add_argument('--unlabeled', action='store_true', # unlabeled deployment data
                        help='Use UnlabeledSharkBody dataset (no masks)')
...
def predict(model, dataset, device, output_dir):
    ...
    with torch.no_grad(): 
        for i, sample in enumerate(dataset):
            if sample is None: 
                continue # skip missing centerpoint entries 
            ...
            img_path = Path(output_dir) / f"img_{filename_root}.png" 
            pred_path = Path(output_dir) / f"pred_{filename_root}.png"

            #if pred_path.exists():
                #logging.info(f"Skipping {filename_root} (prediction already exists)")
                #continue
...
if __name__ == '__main__':
    ...
    # load model weights from a checkpoint if provided via command-line
    if args.checkpoint:
    ...
    # data loader
    if args.unlabeled:
        dataset = UnlabeledSharkBody(cfg)
        logging.info("Using UnlabeledSharkBody dataset for prediction.")
    else:
        dataset = SharkBody(cfg, split=args.split)
        logging.info(f"Using SharkBody dataset for split: {args.split}")

    output_dir = Path(cfg['path_checkpoints']) / 'predictions'
    output_dir.mkdir(parents=True, exist_ok=True) 

    # Run the predictions
    predict(model, dataset, device, output_dir)
